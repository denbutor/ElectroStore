...
# Кешування категорії з продуктами по імені
async def cache_category_products_by_name(category_name: str, products: list):
    redis = await get_redis()

    # ✅ Перетворюємо SQLAlchemy → Pydantic → JSON
    product_schemas = [
...
# Отримання категорії з продуктами по імені
